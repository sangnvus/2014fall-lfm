...
<%@ Import Namespace="LFMS" %>
<%@ Import Namespace="LFMS.Models.BLO" %>
...
<script runat="server">
    ...
    public override void SetConfig()
    {
        ...
        Case cases = caseBLO.GetCaseByCaseCode(Session["caseCode"].ToString());
        var status = "";
        if (cases != null)
        {
            status = cases.Status;
        ...
        {
            BaseUrl = "/Files/" + Session["caseCode"];
        }
        // The phisical directory in the server where the file will end up. If
        // blank, CKFinder attempts to resolve BaseUrl.
        BaseDir = HttpContext.Current.Server.MapPath(BaseUrl);
        ...
        {
            var curId = Session["caseId"].ToString();
            var authorList = (List<string>)Session["UserAuthorize"];
            if (authorList.IndexOf(curId) != -1)
            {
                write = true;
            }
            read = true;
        ...
        if ("Đã thụ lý".Equals(status) && Session["RoleId"].ToString() != "1")
        {
            write = false;
        }
        AccessControl ac2 = AccessControl.Add();
        ...
        if (write)
        {
            ac2.FolderCreate = true;
            ac2.FolderRename = true;
            ac2.FolderDelete = true;
            ac2.FileUpload = true;
            ac2.FileRename = true;
            ac2.FileDelete = true;
        }
        ...
        type = ResourceType.Add("Clips");
        type.Url = BaseUrl + "/clips/";
        type.Dir = BaseDir == "" ? "" : BaseDir + "/clips/";
        type.MaxSize = 1173741824;
        type.AllowedExtensions = new string[] { "swf", "flac", "fla", "mkv", "flv", "avi", "mid", "mpc", "mov", "mpeg", "mpg", "mp4", "mp3", "arm", "wav", "wma", "wmv" };
        ...
        type.MaxSize = 1173741824;
        type.AllowedExtensions = new string[] { "7z", "rar", "zip", "csv", "doc", "docx", "ods", "odt", "pdf", "ppt", "pptx", "rtf", "iso", "txt", "vsd", "xls", "xlsx","tif" };
        type.DeniedExtensions = new string[] { };

        type = ResourceType.Add("Images");
        type.Url = BaseUrl + "/images/";
        type.Dir = BaseDir == "" ? "" : BaseDir + "/images/";
        type.MaxSize = 10737418;
        type.AllowedExtensions = new string[] { "bmp", "gif", "jpeg", "jpg", "JPG", "png" };
        type.DeniedExtensions = new string[] { };
